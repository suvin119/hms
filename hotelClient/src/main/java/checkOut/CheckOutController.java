...
public class CheckOutController {
    ...
    private BookingInfo loadBookingFromServer(int roomId) {
        ...
        for (String row : rows) {
            ...
            if (parts.length > 4) customerName = parts[4];
            
            if (parts.length > 6) {
                try {
                    checkInDate = LocalDate.parse(parts[5]);  // 서버에서 준 체크인
                    checkOutDate = LocalDate.parse(parts[6]); // 서버에서 준 체크아웃
                } catch (Exception e) {
            ...
            if (id == roomId && status.equals("투숙중")) {
                ...
                return new BookingInfo(roomId, customerName, checkInDate, checkOutDate, baseRate, 0, extras);
            }
        }
        return null;
    }
    ...
    private List<ExtraChargeInfo> loadExtraServicesFromServer(int roomId) {
        ...
        System.out.println(">> [부대서비스 조회 결과] 방 " + roomId + " : " + response);
    ...
    private String sendServerRequest(String msg) {
        try (Socket socket = new Socket(SERVER_IP, SERVER_PORT); BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream())); PrintWriter out = new PrintWriter(socket.getOutputStream(), true)) {
